...
#   measurement_types <- paste(mapply(sQuote, measurement_types), collapse=", ")
  ...
  if (nrow(elisa_df) > 0) {
    ...
    elisa_df$elapsed_time_of_specimen_collection = mapply(covertElaspsedTimeToISO8601Format, elisa_df$study_time_of_specimen_collection, 
                                                          elisa_df$unit_of_study_time_of_specimen_collection)
    
    elisa_df$time_point_reference = mapply(getTimePointReference, elisa_df$study_time_t0_event, 
                                         elisa_df$study_time_t0_event_specify)
